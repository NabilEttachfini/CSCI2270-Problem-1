...
void Graph::BFTraversalLabel(std::string startingCity, int distID)
{
 ...
 while (q.empty() != true)
 {
   //  start = q.front();
     ...
     for(int i=0;i<u->adj.size();i++) // adj just adjust size, inlcuded in one of the libraries CHANGE U BACK TO SIZE HERE!!! if does not work
     {
       if(u->adj[i].v->visited==false) //CHANGE U BACK TO SIZE HERE TOO if it does not work!
         {
